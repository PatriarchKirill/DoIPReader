...
class SVITool:
    ...
    def show_upgrade(self):
        ...
        ttk.Button(left_button_frame, text="Start", command=self.start_upgrade).pack(side='left', padx=1)
        
        # Правая группа кнопок
        right_button_frame = ttk.Frame(button_frame)
        right_button_frame.pack(side='right', padx=1)
        
        ttk.Button(right_button_frame, text="Real Time Monitoring").pack(side='left', padx=1)
        ttk.Button(right_button_frame, text="Start LOG").pack(side='left', padx=1)
        ttk.Button(right_button_frame, text="Stop LOG").pack(side='left', padx=1)
        ...
        self.load_upgrade_text(self.upgrade_file)
    ...
    def load_upgrade_file(self):
        ...
        if file_path:
            try:
                # Целевая папка
                target_dir = os.path.join(self.script_dir, "bin", "cash", "cash_upgrade", "work_upb")
    ...
    def export_upgrade_file(self):
        ...
        if not os.path.exists(self.upgrade_file):
        ...
        if target_path:
            try:
                shutil.copy(self.upgrade_file, target_path)
    ...
    def start_upgrade(self):
        ...
        try:
            ...
            Thread(target=self.run_upgrade_script, args=(self.upgrade_script,)).start()
            messagebox.showinfo("Успех", "Процесс обновления запущен")
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось запустить скрипт:\n{str(e)}")

    def run_upgrade_script(self, script_path):
        """Запускает скрипт обновления с отображением прогресса"""
        try:
            self.upgrade_progress["value"] = 0
            self.root.update()
            
            # Здесь должна быть логика выполнения скрипта и обновления прогресса
            # В качестве примера - имитация прогресса
            for i in range(1, 101):
                time.sleep(0.05)  # Имитация работы
                self.upgrade_progress["value"] = i
                self.root.update()
                
            messagebox.showinfo("Успех", "Обновление завершено успешно")
        except Exception as e:
            messagebox.showerror("Ошибка", f"Ошибка при выполнении скрипта:\n{str(e)}")
        finally:
            self.upgrade_progress["value"] = 0
